...
def check_ip_authentication(ip_address, pastebin_url):
    ...
    if response.status_code == 200 and ip_address in response.text:
        ...
        def read_target_usernames():
            with open('target.txt', 'r') as target_file:
                target_usernames = target_file.read().splitlines()
        ...
        class Instagram():
            def __init__(self):
                ...
                self.running = True
                self.target_usernames = read_target_usernames()  
                self.target_index = 0  # Initialize the target index
                with open('target.txt', 'r') as fd: self.tcount = fd.read().splitlines()
            ...
            def get_random_target(self):
                return random.choice(self.target_usernames)
            ...
            def get_random_target(self):
                with open('target.txt', 'r') as target_file:
                    target_usernames = target_file.read().splitlines()
                return random.choice(target_usernames)


            def run(self):
                curl = pycurl.Curl()
                while self.running and self.target_index < len(self.target_usernames):
                    session = random.choice(self.sessions)
                    user, password, email, emailp, sess, fbid = session.split(":")
                    self.username = self.target_usernames[self.target_index]  # Get the next target from the list
        ...
        print(f"{WHITE}[{BLUE}Ω{WHITE}] ΤΛΓGΞΤ: {len(instagram.tcount)}\n")
        ...
        try:
            while True:
                time.sleep(0.01)
                print(f"{WHITE}[{GREEN}ΣVO{WHITE}] ΛTTΞMPTδ {instagram.attempts} | R/S: {instagram.rs} | RL: {instagram.rl}", end='\r')
        except KeyboardInterrupt:
            instagram.running = False
            print("\r{} Evolution stopped, exiting after {:,} attempts...".format(ERROR, instagram.attempts))
            os._exit(1)
    else:
        print('Not allowed')
